...
describe('Filtering User Flows E2E', () => {
  ...
  it('activates multiple filters and clears them', () => {
    ...
    cy.get('[data-testid="island-select"]').select('Dream');
    cy.get('[data-testid="sex-radio-male"]').click();
    cy.url().should('include', 'species=Adelie,Chinstrap&island=Dream&sex=male'); // Assuming species param format

    // Verify partial dataset
    cy.get('[data-testid="data-table-row"]').should('have.length.lessThan', 344);

    // Clear
    cy.get('[data-testid="clear-filters-button"]').click();
    cy.url().should('eq', '/penguins');
