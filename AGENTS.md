# AGENTS.md - Guidelines for Agentic Coding in Penguins Explorer

## Build, Lint, Test Commands

- **Build**: `npm run build` (runs `tsc && vite build`)
- **Lint**: `npm run lint` (ESLint on JS/TS/JSX/TSX files)
- **Lint Fix**: `npm run lint:fix`
- **Format Check**: `npm run prettier` (Prettier check)
- **Format Fix**: `npm run prettier:fix`
- **Test All**: `npm run test` (Vitest)
- **Test UI**: `npm run test:ui`
- **Test Coverage**: `npm run test:coverage`
- **Single Test**: `vitest path/to/test-file.ts` or `vitest --testNamePattern="test name"`
- **E2E Tests**: `npm run cy:open` (Cypress open) or `npm run cy:test`
- **Style All**: `npm run style:all` (TypeScript + lint + prettier)

Always run `npm run style:all` after changes and before commits.

## Code Style Guidelines

- **Framework/Libs**: React 18, TypeScript 5+, MUI 5, TanStack Router/Query, Vitest for tests, Vite bundler.
- **Formatting (Prettier)**: 2-space indent, 80-char width, semicolons, single quotes, trailing commas (ES5), LF line endings.
- **Linting (ESLint)**: Extends Airbnb TypeScript + Prettier. Key rules: No `console.log` (error), allow `any` sparingly, no prop spreading restrictions, React hooks enforced.
- **TypeScript**: Strict mode enabled, no unused locals, path aliases `@/*` for `src/*`. Use explicit types; infer where possible.
- **Imports**: Absolute paths with `@/` for src (e.g., `import { Component } from '@/components/Component'`). Group: React, third-party, local. No extensions.
- **Naming**: PascalCase for components/files, camelCase for vars/functions, UPPER_SNAKE_CASE for constants. Descriptive names.
- **Error Handling**: Use try-catch for async ops, TanStack Query for API errors. Validate props with TS interfaces. No unhandled promises.
- **Conventions**: Functional components with hooks. No class components. Memoize expensive renders. Follow MUI theming. Tests: 80% coverage goal, unit + integration.

# No Cursor/Copilot rules found. Mimic existing patterns in `src/` for new code.
