...
- `npx vitest run src/components/visualizations/VisualizationPanel.test.tsx` *(initially failed: router context issue)*
- Fixed router dependency by mocking useChartConfig hook in VisualizationPanel test
- `npm run lint:fix` *(fixed 2 linting issues: class member spacing)*
...
- `src/components/visualizations/VisualizationPanel.test.tsx` *(modified: added useChartConfig mock, removed router dependency)*
- `cypress/e2e/user-flows/export-visualizations.cy.ts` *(copied from tests/e2e, updated route)*
- `src/hooks/__tests__/useExport.test.ts` *(auto-fixed: linting spacing)*
- `tests/unit/utils/useExport.test.ts` *(auto-fixed: linting spacing)*
## Change Log
