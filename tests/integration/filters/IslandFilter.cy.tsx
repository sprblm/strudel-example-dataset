describe('Island Filter Integration', () => {
  ...
  it('should display all island options when opened', () => {
    ...
    cy.get('[data-testid="island-option-all"]').should('be.visible').and('contain', 'All');
    cy.get('[data-testid="island-option-biscoe"]').should('be.visible').and('contain', 'Biscoe');
    cy.get('[data-testid="island-option-dream"]').should('be.visible').and('contain', 'Dream');
    cy.get('[data-testid="island-option-torgersen"]').should('be.visible').and('contain', 'Torgersen');
  });
  ...
  it('should filter penguins when island is selected', () => {
    ...
    cy.contains('Showing').then(($countText) => {
      ...
      cy.get('[data-testid="island-filter-feedback"]').should('contain', 'Filtering by: Biscoe');
    });
  });
  ...
  it('should update table data in real-time when island filter changes', () => {
    ...
    cy.get('[data-testid="island-filter-feedback"]').should('contain', 'Filtering by: Torgersen');
  });
  ...
  it('should work in combination with species filter', () => {
    ...
    cy.contains('Showing').then(($adelieCount) => {
      ...
      cy.contains('Showing').should(($combinedCount) => {
        const combinedFilterCount = parseInt($combinedCount.text().match(/\\d+/)[0]);
  ...
  it('should reset to show all penguins when "All" is selected', () => {
    ...
    cy.contains('Showing').then(($filteredCount) => {
      const filteredPenguinCount = parseInt($filteredCount.text().match(/\\d+/)[0]);
  ...
  it('should persist filter state in URL', () => {
    ...
    cy.get('[data-testid="island-filter-feedback"]').should('contain', 'Filtering by: Dream');
  });
